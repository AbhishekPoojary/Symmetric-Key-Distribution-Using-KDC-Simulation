...
def needhamSchroeder(package, packageConnection):
    #receiving the contents from step 1
    ...
    IDb = package[8:16]
    IDbAsInt = int(IDb)       # FIXED: was int(IDa)
    IDbAsBinary = bin(IDbAsInt)[2:].zfill(8)  # FIXED: was IDaAsInt
    nonce = package[16:]
    ...
    Ks = nonceGenerator()
    # Use real timestamp encoded as binary to prevent replay attacks
    # We store epoch seconds modulo 1024 as a 10-bit binary string
    # This keeps the same format as the original nonce but carries time info
    T = bin(int(time.time()) % 1024)[2:].zfill(10)

    print("\n" + "="*60)
    print("  KDC: Needham-Schroeder Protocol Initiated")
    print("="*60)
    print(f"  IDa = {IDa},  IDb = {IDb}")
    print(f"  Alice's Master Key (Ka) = {AsKey}")
    print(f"  Bob's Master Key   (Kb) = {BsKey}")
    print(f"  Generated Session Key Ks = {Ks}")
    print(f"  Timestamp T = {T}")
    print(f"  Nonce N1 = {nonce}")
    print("="*60 + "\n")

    #creating the smaller envelope (for Bob)
    messageToBeEncrypted = Ks + IDaAsBinary + T
    encryptedMessage = library.encrypt(messageToBeEncrypted,BsKey)

    #creating the bigger envelope (for Alice)
